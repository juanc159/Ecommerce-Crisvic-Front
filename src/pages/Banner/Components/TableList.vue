...
const { action, banners, totalPage, lastPage, currentPage, totalData, loading, typeAction } = storeToRefs(storeBanner)
// --- --- END Store --- --- ---
...
const paginationData = computed(() => {
  const firstIndex = banners.value.length ? ((currentPage.value - 1) * totalPage.value) + 1 : 0
  const lastIndex = banners.value.length + ((currentPage.value - 1) * totalPage.value)
...
const changeScreen = async (screen: string, registryId: number | null = null) => {
  ...
  storeBanner.clearFormulario()
  storeBanner.typeAction = screen
  if (registryId) storeBanner.fetchInfo(registryId)
}

// Accion cambio de estado
const changeState = async (object: object, state: number) => {
  storeBanner.changeState(object, state)
}
...
const deleteData = async (id: number) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      await storeBanner.fetchDelete(id)
      await fetchDataList()
    }
  })
}
...
<template>
  <div>
    ...
    <HeaderAlertView title="Listado" sub-title="Banners" icon="mdi-format-list-bulleted" />

    <VContainer class="bg-vwhite" fluid>
      <VDataTable :headers="headers" :items="banners" :items-per-page="rowPerPage" @update:sort-by="handleSortBy">
