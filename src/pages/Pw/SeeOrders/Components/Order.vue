                ...
                <div class="row my-4">
                  <div class="cols-8">
                    <div class="form-group">
                      ...
                      <select class="form-control" v-model="typeSelected" style="cursor: pointer" @change="handleUpdate">
                        <option :value="null">Todos</option>
                        <option v-for="item in  typesState" :value="item.id">{{ item.nombre }}</option>
                      </select>
                    </div>
                  </div>
                </div>
                  ...
                  <table class="table text-white mb-0">
                    ...
                    <tbody>
                      <tr v-for="(item,index) in orders">
                        ...
                        <td>{{ new Date(item.created_at).getFullYear() }}/{{ new Date(item.created_at).getMonth() }}/{{ new Date(item.created_at).getDate() }}</td>
                        <td>{{ new Date(item.deliver_date).getFullYear() }}/{{ new Date(item.deliver_date).getMonth() }}/{{ new Date(item.deliver_date).getDate() }}</td>
